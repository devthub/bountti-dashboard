...
export function BounttiTopbar({
  ...
  return (
    <header className="bg-card flex h-15 shrink-0 items-center gap-2 border-b px-6">
      ...
      <div className="ml-auto flex items-center gap-3">
        <label className="bg-background text-muted-foreground flex h-9 items-center gap-2 rounded-lg border px-3 text-xs font-medium">
          ...
          <select
            value={role}
            onChange={(event) =>
              onRoleChange(event.target.value as BounttiRole)
            }
